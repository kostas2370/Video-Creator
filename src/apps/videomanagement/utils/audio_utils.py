    ...
    for j in gpt_answer["scenes"]:
        if is_sentenced:
            for index, sentence in enumerate(j[search_field]):
                make_scene_speech(voice_model, video.dir_name, video.prompt, sentence[narration_field],
                                  index == len(j[search_field]) - 1)

        else:
            make_scene_speech(voice_model, video.dir_name, video.prompt, j['dialogue'], False)
